...
AVI_CONTROLLER_VERSION=`python version.py -c`
if [ ! -z "$2" ]; then
...
sed -i s/"__controller_version__ =.*$"/"__controller_version__ = \'$AVI_CONTROLLER_VERSION\'"/g avi/$PACKAGES/__init__.py
AVI_PIP_VERSION=`python version.py`
...
sed -i s/"__controller_version__ =.*$"/"__controller_version__ = \'$AVI_CONTROLLER_VERSION\'"/g avi/$1/__init__.py
